// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: github.com/overcyn/mochi/pb/view/tabnavigator/tabnavigator.proto

// This CPP symbol can be defined to use imports that match up to the framework
// imports needed when using CocoaPods.
#if !defined(GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS)
 #define GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS 0
#endif

#if GPB_USE_PROTOBUF_FRAMEWORK_IMPORTS
 #import <Protobuf/GPBProtocolBuffers.h>
#else
 #import "GPBProtocolBuffers.h"
#endif

#if GOOGLE_PROTOBUF_OBJC_VERSION < 30002
#error This file was generated by a newer version of protoc which is incompatible with your Protocol Buffer library sources.
#endif
#if 30002 < GOOGLE_PROTOBUF_OBJC_MIN_SUPPORTED_VERSION
#error This file was generated by an older version of protoc which is incompatible with your Protocol Buffer library sources.
#endif

// @@protoc_insertion_point(imports)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"

CF_EXTERN_C_BEGIN

@class MochiPBImage;
@class MochiPBTabNavigatorTab;

NS_ASSUME_NONNULL_BEGIN

#pragma mark - MochiPBTabNavigatorTabnavigatorRoot

/**
 * Exposes the extension registry for this file.
 *
 * The base class provides:
 * @code
 *   + (GPBExtensionRegistry *)extensionRegistry;
 * @endcode
 * which is a @c GPBExtensionRegistry that includes all the extensions defined by
 * this file and all files that it depends on.
 **/
@interface MochiPBTabNavigatorTabnavigatorRoot : GPBRootObject
@end

#pragma mark - MochiPBTabNavigatorTab

typedef GPB_ENUM(MochiPBTabNavigatorTab_FieldNumber) {
  MochiPBTabNavigatorTab_FieldNumber_Id_p = 1,
  MochiPBTabNavigatorTab_FieldNumber_Title = 2,
  MochiPBTabNavigatorTab_FieldNumber_Icon = 3,
  MochiPBTabNavigatorTab_FieldNumber_SelectedIcon = 4,
  MochiPBTabNavigatorTab_FieldNumber_Badge = 5,
};

@interface MochiPBTabNavigatorTab : GPBMessage

@property(nonatomic, readwrite) int64_t id_p;

@property(nonatomic, readwrite, copy, null_resettable) NSString *title;

@property(nonatomic, readwrite, strong, null_resettable) MochiPBImage *icon;
/** Test to see if @c icon has been set. */
@property(nonatomic, readwrite) BOOL hasIcon;

@property(nonatomic, readwrite, strong, null_resettable) MochiPBImage *selectedIcon;
/** Test to see if @c selectedIcon has been set. */
@property(nonatomic, readwrite) BOOL hasSelectedIcon;

@property(nonatomic, readwrite, copy, null_resettable) NSString *badge;

@end

#pragma mark - MochiPBTabNavigatorTabNavigator

typedef GPB_ENUM(MochiPBTabNavigatorTabNavigator_FieldNumber) {
  MochiPBTabNavigatorTabNavigator_FieldNumber_TabsArray = 1,
};

@interface MochiPBTabNavigatorTabNavigator : GPBMessage

@property(nonatomic, readwrite, strong, null_resettable) NSMutableArray<MochiPBTabNavigatorTab*> *tabsArray;
/** The number of items in @c tabsArray without causing the array to be created. */
@property(nonatomic, readonly) NSUInteger tabsArray_Count;

@end

NS_ASSUME_NONNULL_END

CF_EXTERN_C_END

#pragma clang diagnostic pop

// @@protoc_insertion_point(global_scope)
